...
module sweep_and_prune
  ...
  ! DO_K is true for 3D, false for 2D
  LOGICAL, PARAMETER :: DO_K = .TRUE.
  ! NO_K is true for 2D, false for 3D
  LOGICAL, PARAMETER :: NO_K = .FALSE.
...
contains
   ...
   !                                                                      !
   !  Purpose: print a box, for debugging                                 !
   !                                                                      !
   !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
   ...
   subroutine print_box(this,box_id)
    ...
    print *,"================================  vals of ",box_id," are ", &
         this%x_endpoints(this%boxes(box_id)%minendpoint_id(1))%value, &
         this%y_endpoints(this%boxes(box_id)%minendpoint_id(2))%value, &
         this%z_endpoints(this%boxes(box_id)%minendpoint_id(3))%value
    print *,"================================  vals of ",box_id," are ", &
         this%x_endpoints(this%boxes(box_id)%maxendpoint_id(1))%value, &
         this%y_endpoints(this%boxes(box_id)%maxendpoint_id(2))%value, &
         this%z_endpoints(this%boxes(box_id)%minendpoint_id(3))%value
   ...
   !  Subroutine: check                                                   !
   !                                                                      !
   !  Purpose: check the whole table, for debugging                       !
   !                                                                      !
   !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
   ...
   subroutine check(this)
    ...
    do ii = 1, this%x_endpoints_len
       ...
       if (ii .ne. this%boxes(-this%x_endpoints(ii)%box_id)%minendpoint_id(1)  ) then
          ...
          print *,"this%boxes(this%x_endpoints(ii)%box_id)%minendpoint_id(1)", &
               this%boxes(-this%x_endpoints(ii)%box_id)%minendpoint_id(1)
          stop __LINE__
       endif
else
       if (ii .ne. this%boxes(this%x_endpoints(ii)%box_id)%maxendpoint_id(1)  ) then
          ...
          print *,"this%x_endpoints(ii)%box_id is ",this%x_endpoints(ii)%box_id
          print *,"this%boxes(this%x_endpoints(ii)%box_id)%minendpoint_id(1)", &
               this%boxes(this%x_endpoints(ii)%box_id)%maxendpoint_id(1)
    ...
    do ii = 1, this%y_endpoints_len
       if (0 > this%y_endpoints(ii)%box_id) then
          if (ii .ne. this%boxes(-this%y_endpoints(ii)%box_id)%minendpoint_id(2)  ) then
             ...
             print *,"this%boxes(this%y_endpoints(ii)%box_id)%minendpoint_id(1)", &
                  this%boxes(-this%y_endpoints(ii)%box_id)%minendpoint_id(2)
    ...
    do ii=1, this%x_endpoints_len
       ...
       if ( minmax < 0) then
          ...
          do ai=1, list_get_length(active)
             ...
             if (max(this%boxes(aa)%minendpoint_id(2),this%boxes(-minmax)%minendpoint_id(2)) <= &
                  min(this%boxes(-minmax)%maxendpoint_id(2),this%boxes(aa)%maxendpoint_id(2)) .and. &
                  (NO_K .or. max(this%boxes(aa)%minendpoint_id(3),this%boxes(-minmax)%minendpoint_id(3)) <= &
                  min(this%boxes(-minmax)%maxendpoint_id(3),this%boxes(aa)%maxendpoint_id(3)))) then
