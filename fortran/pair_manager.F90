...
module pair_manager
   ...
   ! Initial size of the hashtable (arbitrarily choose first prime over 10000)
   INTEGER, PARAMTER :: INITIAL_SIZE = 10006
...
contains
  ...
  subroutine init_pairs(this)
    ...
    this%current_hash = 0
    if (.not.allocated(this%table)) allocate(this%table(0:INITIAL_SIZE))
    this%table(:)%ii = 0
    this%table(:)%jj = 0
    this%table_size = 0
